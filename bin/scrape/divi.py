...
def to_dataframe(mapping: Callable[[str], float] = TRANSLATE_AVAILABILITY.get) -> pd.DataFrame:
    ...
    for row in response_json['data']:
        ...
        columns[headers[2]].append(mapping(row['maxBettenStatusEinschaetzungLowCare']))
        columns[headers[3]].append(mapping(row['maxBettenStatusEinschaetzungHighCare']))
        columns[headers[4]].append(mapping(row['maxBettenStatusEinschaetzungEcmo']))
        columns[headers[5]].append(row['letzteMeldezeitpunkt'])
